...
use `order-directory`;
/*1) To /*create Tables for the supplier,customer,category,product,product_Details,
order,rating ;to store the data for the E-commerce order directory */
...
#Queries:
/*3) To Display the number of the customer group by their genders 
who have placed any order of amount greater than or equal to Rs.3000 */
select count(t2.cus_gender) as NoOfCustomers,t2.cus_gender
 from(select t1.cus_id,t1.cus_gender,t1.ord_amount,t1.cus_name   
from(select `order`.ord_amount,`order`.cus_id,customer.cus_gender,customer.cus_name 
from `order`
inner join
customer
on `order`.cus_id=customer.cus_id having `order`.ord_amount>=3000) as t1
...
inner join
supplier_pricing on supplier_pricing.pro_id=product.pro_id ) as A1 on 
...
select category.cat_id,category.cat_name,min(t3.min_price) as
 ...
 where t2.pro_id=product.pro_id)as t3 where 
 t3.cat_id=category.cat_id group by t3.cat_id;
 ...
 product.pro_id=supplier_pricing.pro_id where
 ...
 customer where customer.cus_name like 'A%' or cus_name like '%A';
 /*9 To Create a stored procedure to display the Rating for a Supplier 
